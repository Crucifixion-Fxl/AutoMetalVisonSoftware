...
namespace AutoMetal.Algorithms
{
    public class Yolov8Seg
    {
        // 定义类别数量 或许可能要更改
        public int CategNums = 1;
        // 检测的置信度阈值
        public float DetThresh = 0.5f;
        // NMS的过程的置信度阈值， 超过这个值的框不会被保留
        public float DetNmsThresh = 0.5f;
        // 归一化因子
        public float[] Factors;
        // 输入的维度 （b,c,h,w）
        public Dims InputDims;
        // 模型输出检测头的预测框数量，模型默认输出8400个框
        // 每个框包含了4个坐标值，1个置信度值和80个类别的概率值
        ...
        public Yolov8Seg(string enginePath)
        {
            ...
            predictor = new Nvinfer(enginePath);

            // 获取YOLOv8模型输入层绑定的维度信息
        ...
        public List<SegResult> Predict(List<Mat> images)
        {
            ...
            for(int begImgNo = 0;begImgNo < images.Count;begImgNo+= BatchNum)
            {
                ...
                Slog.INFO("Inference time: " + (end - start).TotalMilliseconds + " ms.");
                start = DateTime.Now;
        ...
        public List<SegResult> ProcessResult(float[] detect, float[] proto, int batch)
        {
            ...
            {
                Mat detectData = Mat.FromPixelData(36 + CategNums, OutputLength, MatType.CV_32FC1,
                     Marshal.UnsafeAddrOfPinnedArrayElement(detect, (4 + CategNums + 32) * OutputLength * b));
                Mat protoData = Mat.FromPixelData(32, MaskLength * MaskLength, MatType.CV_32F,
